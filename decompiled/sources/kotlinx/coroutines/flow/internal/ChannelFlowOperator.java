...
public abstract class ChannelFlowOperator<S, T> extends ChannelFlow<T> {
    ...
    public final Object collectWithContextUndispatched(FlowCollector<? super T> flowCollector, CoroutineContext newContext, Continuation<? super Unit> continuation) {
        FlowCollector originalContextCollector = ChannelFlowKt.access$withUndispatchedContextCollector(flowCollector, continuation.getContext());
        return ChannelFlowKt.withContextUndispatched$default(newContext, originalContextCollector, null, new ChannelFlowOperator$collectWithContextUndispatched$2(this, null), continuation, 4, null);
    }
    ...
    static /* synthetic */ <S, T> Object collect$suspendImpl(ChannelFlowOperator<S, T> channelFlowOperator, FlowCollector<? super T> flowCollector, Continuation<? super Unit> continuation) {
        if (channelFlowOperator.capacity == -3) {
            ...
            if (Intrinsics.areEqual(newContext, collectContext)) {
                ...
                return flowCollect == IntrinsicsKt.getCOROUTINE_SUSPENDED() ? flowCollect : Unit.INSTANCE;
            } else if (Intrinsics.areEqual(newContext.get(ContinuationInterceptor.Key), collectContext.get(ContinuationInterceptor.Key))) {
