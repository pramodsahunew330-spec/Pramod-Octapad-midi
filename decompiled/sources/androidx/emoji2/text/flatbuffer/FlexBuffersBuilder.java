    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        if (!fixed || typed) {
            ...
            for (int i4 = start; i4 < this.stack.size(); i4++) {
                int elemWidth = this.stack.get(i4).elemWidth(this.bb.writePosition(), i4 + prefixElems);
            ...
            for (int i5 = start; i5 < this.stack.size(); i5++) {
                writeAny(this.stack.get(i5), byteWidth);
            }
            if (!typed) {
                for (int i6 = start; i6 < this.stack.size(); i6++) {
                    this.bb.put(this.stack.get(i6).storedPackedType(bitWidth));
                }
            }
            if (keys != null) {
                i2 = 9;
            ...
            return new Value(key, i2, bitWidth, vloc);
        }
        throw new AssertionError();
    }
