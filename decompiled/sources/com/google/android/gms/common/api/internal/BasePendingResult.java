...
public abstract class BasePendingResult<R extends Result> extends PendingResult<R> {
    ...
    public void cancel() {
        synchronized (this.zae) {
            if (!this.zam && !this.zal) {
                ...
                zal(this.zaj);
                this.zam = true;
                zab(mo340createFailedResult(Status.RESULT_CANCELED));
            }
        }
    }

    /* renamed from: createFailedResult */
    public abstract R mo340createFailedResult(Status status);

    @Deprecated
    public final void forceFailureUnlessReady(Status status) {
        synchronized (this.zae) {
            if (!isReady()) {
                setResult(mo340createFailedResult(status));
                this.zan = true;
            }
        }
    }
    ...
    @Override // com.google.android.gms.common.api.PendingResult
    public final void setResultCallback(ResultCallback<? super R> resultCallback, long time, TimeUnit units) {
        synchronized (this.zae) {
            if (resultCallback == null) {
                this.zah = null;
                return;
            ...
            if (isReady()) {
                ...
                callbackHandler.sendMessageDelayed(callbackHandler.obtainMessage(2, this), units.toMillis(time));
            }
        }
    }

    public BasePendingResult(GoogleApiClient apiClient) {
        this.zae = new Object();
        this.zaf = new CountDownLatch(1);
        this.zag = new ArrayList();
        this.zai = new AtomicReference();
