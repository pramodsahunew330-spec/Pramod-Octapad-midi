...
public class RBTreeSortedMap<K, V> extends ImmutableSortedMap<K, V> {
    ...
    public ImmutableSortedMap<K, V> remove(K key) {
        ...
        LLRBNode<K, V> newRoot = this.root.remove(key, this.comparator).mo548copy(null, null, LLRBNode.Color.BLACK, null, null);
        return new RBTreeSortedMap(newRoot, this.comparator);
    }

    @Override // com.google.firebase.database.collection.ImmutableSortedMap
    public ImmutableSortedMap<K, V> insert(K key, V value) {
        LLRBNode<K, V> newRoot = this.root.insert(key, value, this.comparator).mo548copy(null, null, LLRBNode.Color.BLACK, null, null);
        return new RBTreeSortedMap(newRoot, this.comparator);
    }
    ...
    public static class Builder<A, B, C> {
        ...
        public static class Base1_2 implements Iterable<BooleanChunk> {
            ...
            public Base1_2(int size) {
                int toCalc = size + 1;
                this.length = (int) Math.floor(Math.log(toCalc) / Math.log(2.0d));
                long mask = ((long) Math.pow(2.0d, this.length)) - 1;
        ...
        public static <A, B, C> RBTreeSortedMap<A, C> buildFrom(List<A> keys, Map<B, C> values, ImmutableSortedMap.Builder.KeyTranslator<A, B> translator, Comparator<A> comparator) {
            ...
            return new RBTreeSortedMap<>(((Builder) builder).root == null ? LLRBEmptyNode.getInstance() : ((Builder) builder).root, comparator);
        }
    }
}
