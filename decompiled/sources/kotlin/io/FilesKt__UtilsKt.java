...
public class FilesKt__UtilsKt extends FilesKt__FileTreeWalkKt {
    ...
    public static final boolean copyRecursively(File $this$copyRecursively, File target, boolean overwrite, Function2<? super File, ? super IOException, ? extends OnErrorAction> onError) {
        ...
        if (!$this$copyRecursively.exists()) {
            return onError.mo2029invoke($this$copyRecursively, new NoSuchFileException($this$copyRecursively, null, "The source file doesn't exist.", 2, null)) != OnErrorAction.TERMINATE;
        }
        try {
            ...
            while (it.hasNext()) {
                File src = it.next();
                if (!src.exists()) {
                    if (onError.mo2029invoke(src, new NoSuchFileException(src, null, "The source file doesn't exist.", 2, null)) == OnErrorAction.TERMINATE) {
                        return false;
                    }
                } else {
                    ...
                    if (dstFile.exists() && (!src.isDirectory() || !dstFile.isDirectory())) {
                        ...
                        if (stillExists) {
                            if (onError.mo2029invoke(dstFile, new FileAlreadyExistsException(src, dstFile, "The destination file already exists.")) == OnErrorAction.TERMINATE) {
                                return false;
                            }
                        }
                    }
                    ...
                    if (stillExists2) {
                        dstFile.mkdirs();
                    } else if (FilesKt.copyTo$default(src, dstFile, overwrite, 0, 4, null).length() != src.length() && onError.mo2029invoke(src, new IOException("Source file wasn't copied completely, length of destination file differs.")) == OnErrorAction.TERMINATE) {
                        return false;
                    }
                }
            }
