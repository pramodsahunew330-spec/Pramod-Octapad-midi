...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m2213plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m2122toLongimpl(duration, getUnit());
        boolean z = true;
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m2212overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m2079divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m2122toLongimpl(half, getUnit());
        ...
        if (!z) {
            ...
            try {
                m2213plusAssignLRDsOJo(half);
                m2213plusAssignLRDsOJo(Duration.m2111minusLRDsOJo(duration, half));
                return;
    ...
    private final void m2212overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m2125toStringimpl(duration)) + '.');
    }
}
